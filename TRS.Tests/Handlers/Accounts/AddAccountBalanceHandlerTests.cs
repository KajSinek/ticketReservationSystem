...
public class AddAccountBalanceHandlerTests : TestBase
{
    ...
    public async Task AddAccountBalance_ExpectedResult() {
        ...
        var request = new AccountBalanceHandlerCommand
        {
            AccountId = accountId,
            Value = 100
        };

        var accountBalanceResponse = new EntityResponse<AccountBalance>
        {
        ...
        _baseDbRequests.GetEntityByPropertyAsync(Arg.Any<GetEntityByPropertyModel<AccountBalance>>()).Returns(accountBalanceResponse);
        _baseDbRequests.UpdateAsync(Arg.Any<Guid>(), Arg.Any<AccountBalance>()).Returns(updatedAccountBalanceResponse);
