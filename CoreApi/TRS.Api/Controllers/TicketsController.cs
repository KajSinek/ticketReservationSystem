...
public class TicketsController(IMediator mediator) : ControllerBase
{
    ...
    public async Task<IActionResult> CreateTicketAsync(
        [FromBody, Bind] CreateTicketModel model,
        CancellationToken ct
    )
    {
        var result = await mediator.Send(
            new CreateTicketHandlerCommand
            {
                Name = model.Name,
                Price = model.Price,
    ...
    public async Task<IActionResult> GetTicketAsync(
        [Required, FromRoute(Name = "ticket_id")] Guid ticket_id,
        CancellationToken ct
    )
    {
    ...
    public async Task<IActionResult> UpdateTicketAsync(
        [Required, FromRoute(Name = "ticket_id")] Guid ticket_id,
        [FromBody, Bind] UpdateTicketModel model,
        CancellationToken ct
    )
    {
        var result = await mediator.Send(
            new UpdateTicketHandlerCommand
            {
                TicketId = ticket_id,
                Name = model.Name,
                Price = model.Price,
    ...
    public async Task<IActionResult> DeleteTicketAsync(
        [Required, FromRoute(Name = "ticket_id")] Guid ticket_id,
        CancellationToken ct
    )
    {
        var result = await mediator.Send(
            new DeleteTicketHandlerCommand { TicketId = ticket_id },
            ct
        );
