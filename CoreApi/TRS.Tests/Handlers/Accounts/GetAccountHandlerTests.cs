...
public class GetAccountHandlerTests : TestBase
{
    ...
    public async Task Handle_WhenCalled_ReturnsAccount()
    {
        ...
        var query = new GetAccountHandlerQuery { AccountId = accountId };

        var response = new EntityResponse<Account> { Entity = account };
    ...
    {
        // Arrange
        var query = new GetAccountHandlerQuery { AccountId = Guid.NewGuid() };
